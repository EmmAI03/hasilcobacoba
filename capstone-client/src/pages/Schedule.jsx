...
const initialEvents = {
  "2025-08-01": [
    { id: 1, title: "API Key Issue", color: "bg-rose-500", time: "10:00" },
  ],
  "2025-08-03": [
    { id: 2, title: "Develop Homepage", color: "bg-amber-500", time: "14:00" },
  ],
  "2025-08-05": [
  ...
  "2025-08-08": [
    { id: 4, title: "User Auth Setup", color: "bg-sky-500", time: "11:00" },
  ],
  "2025-09-10": [
    {
      id: 5,
  ...
  "2025-01-20": [
    { id: 6, title: "Q1 Planning", color: "bg-teal-500", time: "09:30" },
  ],
  "2025-08-06": [
    {
      id: 7,
...
const CalendarDay = ({ day, onClick, onEventDrop }) => {
  ...
  return (
    <div
      className={`border border-slate-200 h-28 p-2 flex flex-col transition-colors ${
...
const CalendarEvent = ({ event }) => (
  <div
    className={`p-1 text-xs font-semibold rounded-md text-white ${event.color} mb-1 truncate cursor-grab`}
  >
    {event.title}
  </div>
);
...
const WeekView = ({ currentDate, events, onEventDrop }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 grid grid-cols-7">
      {weekDays.map((day) => (
        <WeekDayColumn key={day.date} day={day} onEventDrop={onEventDrop} />
      ))}
    </div>
  );
};
...
const MiniCalendar = ({ monthDate, events, onClick }) => {
  ...
  return (
    <div
      className="border border-slate-200 rounded-lg p-3 cursor-pointer hover:border-sky-500"
      onClick={onClick}
    >
    ...
    >
      ...
      >
        <div className="p-6 border-b">
          <h3 className="text-lg font-bold">Schedule for {fullDateStr}</h3>
        </div>
        ...
        <div className="p-4 bg-slate-50 text-right">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-slate-200 font-semibold rounded-lg"
          >
            Close
          </button>
        </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 bg-slate-100 p-1 rounded-lg">
              {["day", "week", "month", "year"].map((v) => (
                <button
                  key={v}
                  onClick={() => setView(v)}
                  className={`px-3 py-1 text-sm font-semibold rounded-md capitalize transition-all ${
                    view === v
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-4 py-2 text-sm font-semibold border border-slate-300 rounded-lg"
              >
                Today
              </button>
              <button
