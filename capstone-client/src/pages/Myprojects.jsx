...
const Modal = ({ isOpen, onClose, title, children }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl shadow-2xl p-6 sm:p-8 w-11/12 max-w-md relative animate-fade-in-up"
...
const initialProjects = [
  {
    ...
    tasks: {
      ...
      Done: [
        { id: "t1-3", title: "Gather user feedback", endDate: "2025-08-15" },
      ],
      Stuck: [],
    },
  },
  {
    ...
    tasks: {
      ...
      Ongoing: [
        {
          id: "t2-1",
          title: "Finalize App Store assets",
          endDate: "2025-08-10",
        },
      ],
      Done: [
        { id: "t2-2", title: "Beta testing phase 1", endDate: "2025-07-30" },
        {
          id: "t2-3",
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white/80 backdrop-blur-sm sticky top-0 z-10 p-4 border-b border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button
              onClick={onBack}
              className="p-2 rounded-md hover:bg-slate-200 flex items-center gap-2 text-slate-600 transition-colors"
            >
          ...
          <button
            ...
            className="flex items-center gap-2 bg-sky-500 text-white px-4 py-2 rounded-lg font-semibold hover:bg-sky-600 transition-transform hover:scale-105 text-sm"
          >
...
const ProjectListCard = ({ project, onProjectClick }) => {
  const allTasks = useMemo(
    () => Object.values(project.tasks).flat(),
    [project.tasks]
  );
  ...
  return (
    <div
      ...
      className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 hover:shadow-lg hover:border-sky-400 transition-all cursor-pointer animate-fade-in-up"
    >
      ...
      <div className="flex items-center gap-3">
        <div className="w-full bg-slate-200 rounded-full h-2.5">
          <div
            className={`${progressColorClass} h-2.5 rounded-full transition-all duration-500`}
...
const NewProjectForm = ({ onAddProject }) => {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <button
        type="submit"
        className="w-full bg-sky-500 text-white py-2.5 rounded-lg font-semibold hover:bg-sky-600 transition-colors"
      >
        Create Project
      </button>
    </form>
    ...
    <form onSubmit={handleSubmit}>
      <div className="mb-4">
        ...
        <input
          ...
          onChange={(e) => setTitle(e.target.value)}
          className="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-sky-500"
          placeholder="e.g., Draft initial design mockups"
          required
        />
      </div>
      <div className="mb-6">
