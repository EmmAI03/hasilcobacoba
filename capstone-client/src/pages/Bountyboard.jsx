...
const initialBounties = [
    { id: 1, title: 'Real-time Chat Application', description: 'Build a real-time chat app using WebSockets and React.', tags: [{ name: 'Socket.io', color: 'bg-sky-100 text-sky-700' }, { name: 'WebSocket', color: 'bg-blue-100 text-blue-700' }, { name: 'Realtime', color: 'bg-green-100 text-green-700' }], reward: 350, dueDate: '2025-08-25' },
    { id: 2, title: 'Build Responsive Landing Page', description: 'Create a mobile-friendly landing page using React and Tailwind CSS.', tags: [{ name: 'React', color: 'bg-cyan-100 text-cyan-700' }, { name: 'Tailwind', color: 'bg-teal-100 text-teal-700' }, { name: 'Frontend', color: 'bg-indigo-100 text-indigo-700' }], reward: 300, dueDate: '2025-08-10' },
    { id: 3, title: 'E-commerce Product Page', description: 'Develop a product page with dynamic content and user reviews.', tags: [{ name: 'React', color: 'bg-cyan-100 text-cyan-700' }, { name: 'API', color: 'bg-purple-100 text-purple-700' }, { name: 'Design', color: 'bg-pink-100 text-pink-700' }], reward: 280, dueDate: '2025-08-20' },
    { id: 4, title: 'REST API for Task Management', description: 'Design and develop a RESTful API for a task management system.', tags: [{ name: 'Node.js', color: 'bg-lime-100 text-lime-700' }, { name: 'Express', color: 'bg-gray-200 text-gray-700' }, { name: 'Backend', color: 'bg-stone-200 text-stone-700' }], reward: 250, dueDate: '2025-08-05' },
    { id: 5, title: 'UI Redesign for Dashboard', description: 'Improve usability and aesthetics of existing dashboard UI.', tags: [{ name: 'UX', color: 'bg-yellow-100 text-yellow-700' }, { name: 'UI', color: 'bg-orange-100 text-orange-700' }, { name: 'Figma', color: 'bg-red-100 text-red-700' }], reward: 200, dueDate: '2025-08-15' },
    { id: 6, title: 'UI Settings Page', description: 'Create a settings page for user preferences and configurations.', tags: [{ name: 'React', color: 'bg-cyan-100 text-cyan-700' }, { name: 'Settings', color: 'bg-slate-200 text-slate-700' }, { name: 'Frontend', color: 'bg-indigo-100 text-indigo-700' }], reward: 180, dueDate: '2025-08-30' },
];

// --- Bounty Card Component ---
const BountyCard = ({ bounty, onSelectBounty }) => {
    return (
        <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6 flex flex-col transition-all duration-300 hover:shadow-xl hover:scale-[1.02]">
            ...
            <div className="mt-6 border-t border-slate-200 pt-4">
                 <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => onSelectBounty(bounty)} className="px-5 py-2 bg-indigo-600 text-white rounded-lg text-sm font-semibold hover:bg-indigo-700 transition-colors shadow-sm hover:shadow-md">View Details</button>
                </div>
            </div>
        </div>
    );
                ...
                {selectedBounty && (
                    <div>
                        <div className="flex justify-between items-start">
                             ...
                             <span className="text-2xl font-bold text-green-600">${selectedBounty.reward}</span>
                        </div>
                        ...
                        <div className="flex justify-end gap-3 mt-6 border-t border-slate-200 pt-6">
                            <button onClick={() => setSelectedBounty(null)} className="px-5 py-2 bg-slate-200 rounded-lg font-semibold text-slate-800 hover:bg-slate-300 transition-colors">Close</button>
                             <button className="px-5 py-2 bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 transition-colors shadow-sm">Claim Bounty</button>
                        </div>
                    </div>
                )}
            </Modal>
